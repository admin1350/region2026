<img width="967" height="635" alt="изображение" src="https://github.com/user-attachments/assets/29a82c3a-18f0-427b-adb5-7fb62d72c161" /># Модуль Б. Настройка технических и программных средств информационно-коммуникационных систем
...
# 7. Настройка имён и IP-адресации на устройствах rtr-cod и rtr-a
## Вариант реализации:
### sw1-a (alt-server):
#### Назначение имени на устройство:
...
echo "nameserver 77.88.8.8" > /etc/resolv.conf
```
* Поcле чего обновляем список пакетов и устанавливаем openvswitch:
```
apt-get update && apt-get install -y openvswitch
```
* Включаем и добавляем в автозагрузку openvswitch:
...
![](photo/8.fw-cod10.png)

* Проверить можно на вкладке Подробности:

![](photo/8.fw-cod11.png)

#### Доступ к веб-интерфейсу урпавления fw-cod:
* Открываем браузер и переходи по https://172.16.1.2:8443 (IP-адрес fw-cod):
